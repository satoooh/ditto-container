...
class EyeAttrUtilsByMP:
    def __init__(self, lmks_mp):
        ...
        self.L_ball_direc = (self.L_ball - self.L_center) / (
            self.L_width[:, None] + 1e-8
        )  # [n, 3/2]
        self.R_ball_direc = (self.R_ball - self.R_center) / (
            self.R_width[:, None] + 1e-8
        )
        ...
        self.L_ball_move_direc = (
            self._direc(self.L_ball, self.L_center) - self.L_eye_direc
        )
        self.R_ball_move_direc = (
            self._direc(self.R_ball, self.R_center) - self.R_eye_direc
        )
    ...
    def LR_ball_direc(self):
        LR_ball_direc = np.stack(
            [self.L_ball_direc, self.R_ball_direc], -1
        )  # [n, 3, 2]
