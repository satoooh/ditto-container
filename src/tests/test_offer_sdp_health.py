# Where: src/tests/test_offer_sdp_health.py
# What: Verify /webrtc/offer returns SDP answer and recvonly transceivers are created; root health 200 (task 5.1).
...
class FakePC:
    ...
    def addTransceiver(self, kind, direction="sendrecv"):
        tx = FakeTransceiver(kind, direction)
...
def test_offer_returns_sdp_and_recvonly_transceivers(client, tmp_path):
    server, api = client
    ...
    # transceivers should be recvonly
    for pc in server._pcs:
        assert any(tx.kind == "video" and tx.direction == "recvonly" for tx in pc.transceivers)
        assert any(tx.kind == "audio" and tx.direction == "recvonly" for tx in pc.transceivers)
