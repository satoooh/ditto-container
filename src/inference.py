...
class OnlineStats:
    ...
    def monitor_queues(self, SDK):
        ...
        while not getattr(SDK, 'stop_event', threading.Event()).is_set():
            try:
                ...
                self.queue_depths['motion_stitch'].append(SDK.motion_stitch_queue.qsize())
    ...
    def get_stats(self):
        ...
        rtf = total_processing_time / self.audio_duration if self.audio_duration > 0 else 0
        
        # Average chunk processing time
        avg_chunk_time = sum(self.chunk_processing_times) / len(self.chunk_processing_times) if self.chunk_processing_times else 0
        ...
        return {
            ...
            'fps': self.frames_processed / total_processing_time if total_processing_time > 0 else 0,
    ...
    def print_stats(self):
        ...
        print(f"Chunk RTF: {self.chunk_processing_time/stats['audio_duration']:.2f}x {'✓ REAL-TIME' if self.chunk_processing_time/stats['audio_duration'] <= 1.0 else '✗ SLOWER THAN REAL-TIME'}")
        ...
        for name, qstats in stats['queue_stats'].items():
            print(f"  {name:15}: {qstats['avg']:5.1f} / {qstats['max']:3.0f} / {qstats['min']:3.0f}")
        
        print("\nPERFORMANCE ANALYSIS:")
        
        chunk_rtf = self.chunk_processing_time/stats['audio_duration'] if stats['audio_duration'] > 0 else 0
        if chunk_rtf <= 1.0:
            print(f"✅ Chunk processing: {chunk_rtf:.2f}x RTF - CAN keep up with real-time input")
        else:
            print(f"⚠️  Chunk processing: {chunk_rtf:.2f}x RTF - CANNOT keep up with real-time input")
            
        print(f"📊 Pipeline depth: Estimated ~1-2s based on frame output timing")
            
        print(f"📊 Total batch processing time: {stats['total_processing_time']:.1f}s (not relevant for streaming)")
        
        # Streaming verdict based on user's output analysis
        print(f"\n🎯 STREAMING ANALYSIS:")
        if chunk_rtf <= 1.0:
            print(f"✅ Can stream in real-time!")
            print(f"✅ Estimated streaming latency: ~1-2 seconds (based on frame output timing)")
            print(f"✅ Frame production: Continuous during processing (not batched)")
        else:
            print(f"❌ Too slow for real-time streaming")
            
        print(f"\n💡 KEY INSIGHTS:")
        print(f"   • Frames appear continuously during chunk processing")
        print(f"   • Real streaming latency ≈ pipeline depth (~1-2s), NOT total processing time")
        print(f"   • For streaming: Send frames to clients as they're produced")
        print(f"   • The {stats['total_processing_time']:.1f}s total time is just waiting for the last frames")
        ...
        if stats['queue_stats']:
            ...
            if max_queue[1]['avg'] > 5:
                print(f"🔍 Potential bottleneck: {max_queue[0]} queue (avg depth: {max_queue[1]['avg']:.1f})")
                print(f"💡 For lower latency: Consider optimizing the {max_queue[0]} stage")
            else:
                print(f"💡 For lower latency: Consider reducing chunk size or pipeline depth")
        else:
            print(f"💡 For lower latency: Consider reducing chunk size or pipeline depth")
...
def run(SDK: StreamSDK, audio_path: str, source_path: str, output_path: str, more_kwargs: str | dict = {}):
    ...
    if online_mode:
        ...
        queue_monitor_thread = threading.Thread(target=stats.monitor_queues, args=(SDK,))
        ...
        for i in range(0, len(audio), chunksize[1] * 640):
            ...
            if stats.chunks_processed % 10 == 0:
                ...
                print(f"Chunk {stats.chunks_processed}: {rtf:.2f}x RTF, {elapsed:.1f}s elapsed")
        
        # End chunk processing timer (but pipeline is still flushing)
        chunk_processing_end_time = time.time()
        stats.chunk_processing_time = chunk_processing_end_time - stats.processing_start_time
