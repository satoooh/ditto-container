...
def build_binary_frame_payload(
    frame_id: int, timestamp: float, jpeg_bytes: bytes
) -> bytes:
...
def parse_binary_frame(payload: bytes) -> Tuple[int, float, bytes]:
    ...
    frame_id, timestamp, length = FRAME_HEADER_STRUCT.unpack(
        payload[: FRAME_HEADER_STRUCT.size]
    )
