...
class BandwidthTester:
    ...
    async def test_server_to_client(self, websocket: WebSocket, client_id: str):
        ...
        for size_kb in test_sizes:
            ...
            if ack == "ack":
                ...
                logger.info(f"✅ {size_kb}KB: {mbps:.2f} Mbps ({duration*1000:.1f}ms)")
            else:
        ...
        try:
            ...
            logger.info(f"✅ Sustained: {sustained_mbps:.2f} Mbps ({total_bytes/(1024*1024):.1f}MB total)")
    ...
    async def run_full_test(self, websocket: WebSocket, client_id: str):
        ...
        try:
            ...
            results["server_to_client"] = await self.test_server_to_client(websocket, client_id)
            
            # Small delay between tests
            await asyncio.sleep(1)
            
            # Test 2: Client to Server  
            results["client_to_server"] = await self.test_client_to_server(websocket, client_id)
    ...
    def print_summary(self, results):
        ...
        if "sustained_10s" in s2c and "mbps" in s2c["sustained_10s"]:
            ...
            print(f"  Current target: 40 fps")
            
            if max_fps >= 40:
                print("  ✅ Network can handle 40 FPS streaming")
            else:
                print(f"  ⚠️  Network limited to {max_fps:.1f} FPS")
                print(f"  💡 Reduce frame size to {int(frame_size_kb * max_fps / 40)} KB for 40 FPS")
...
async def get_homepage():
    return {"message": "Bandwidth Test Server", "endpoint": "/bandwidth_test/{client_id}"}
...
def main():
    ...
    print(f"📡 Test endpoint: ws://{args.host}:{args.port}/bandwidth_test/{{client_id}}")
    print(f"🔧 WebSocket max frame size: 5MB")
