...
class BandwidthTestClient:
    ...
    async def handle_server_to_client_test(self):
        ...
        for size_kb in test_sizes:
            ...
            try:
                ...
                logger.info(f"✅ Received {len(data)} bytes in {duration*1000:.1f}ms = {mbps:.2f} Mbps")
        ...
        try:
            ...
            logger.info(f"✅ Sustained test complete:")
            logger.info(f"   📊 Received: {total_bytes/(1024*1024):.1f} MB in {duration:.1f}s")
    ...
    async def handle_client_to_server_test(self):
        ...
        for size_kb in test_sizes:
            ...
            try:
                ...
                if ack == "ack":
                    ...
                    logger.info(f"✅ Sent {len(test_data)} bytes in {duration*1000:.1f}ms = {mbps:.2f} Mbps")
                else:
        ...
        try:
            ...
            logger.info(f"✅ Sustained upload complete:")
            logger.info(f"   📊 Sent: {total_bytes/(1024*1024):.1f} MB in {duration:.1f}s")
            logger.info(f"   📊 Bandwidth: {sustained_mbps:.2f} Mbps")
            logger.info(f"   📊 Chunks: {chunk_count}")
    ...
    def print_client_summary(self):
        ...
        if s2c:
            for size, data in s2c.items():
                if isinstance(data, dict) and "mbps" in data:
                    print(f"  {size:>10}: {data['mbps']:>8.2f} Mbps ({data['duration_ms']:>6.1f}ms)")
        else:
        ...
        if "sustained_10s" in s2c and "mbps" in s2c["sustained_10s"]:
            ...
            print(f"  Maximum FPS by frame size:")
            ...
            if sustained_mbps >= required_mbps:
                print(f"    Status: ✅ Network supports current streaming")
            else:
                ratio = sustained_mbps / required_mbps
                print(f"    Status: ❌ Network only supports {ratio*100:.0f}% of required bandwidth")
                print(f"    Suggested: Reduce to {int(46 * ratio)}KB frames or {int(40 * ratio)}fps")
        
        print("="*60)

async def main():
    parser = argparse.ArgumentParser(description="Bandwidth Test Client")
    parser.add_argument("--server", required=True, help="WebSocket server URL (e.g., ws://10.49.167.242:8001)")
    parser.add_argument("--client_id", default="test_client", help="Client identifier")
    args = parser.parse_args()
    
    print(f"🚀 Starting Bandwidth Test Client")
