...
- NGC ベースの `nvcr.io/nvidia/tensorrt:25.08-py3` を元に CUDA 12.9 + TensorRT 10.13.3 を利用（画像側の TensorRT をそのまま使用し、pip での上書きは行いません）
...
- Polygraphy CLI を使って旧来のフローを維持したい場合は `--use-polygraphy` を付与するか、環境変数 `DITTO_USE_POLYGRAPHY=1` を設定して実行します
